...
class ApiListingDetailScreen extends StatefulWidget {
  final String postId;
  ...
  @override
  State<ApiListingDetailScreen> createState() => _ApiListingDetailScreenState();
}

class _ApiListingDetailScreenState extends State<ApiListingDetailScreen> {
  ...
  Future<void> _fetchOwnerProfile() async {
    try {
      ...
      if (firebaseUid == null) {
        final title = (widget.post['title'] ?? widget.post['originalPrompt'] ?? '').toString().trim();
        if (title.isNotEmpty) {
          final query = await _firestore
              .collection('posts')
              .where('originalPrompt', isEqualTo: title)
  ...
  String get _smartMessage => (post['smart_message'] ?? '').toString().trim();
  String get _recommendation => (post['recommendation'] ?? '').toString().trim();
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // Description (uses reasoning from API)
                              ...
                              if (_hasProductDetails) ...[
                                ...
                                const SizedBox(height: 10),
                                _buildGlassCard(
                                  child: Column(children: _productDetailRows),
                                ),
                                const SizedBox(height: 16),
                              ],

                              // Smart Message (similar matches only)
                              if (_isSimilarMatch && _smartMessage.isNotEmpty) ...[
                                const Text(
                                  'Match Insight',
                                  style: TextStyle(
                                    fontFamily: 'Poppins',
                              ...
                              // Recommendation (similar matches only)
                              if (_isSimilarMatch && _recommendation.isNotEmpty) ...[
                                const Text(
                                  'Recommendation',
                                  style: TextStyle(
                                    fontFamily: 'Poppins',
                                ...
                                _buildGlassCard(
                                  child: Row(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Icon(Icons.recommend, color: Colors.green[400], size: 20),
                                      const SizedBox(width: 10),
                                      Expanded(
                                        child: Text(
                                          _recommendation,
  ...
  Future<void> _openChat() async {
    ...
    try {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => EnhancedChatScreen(otherUser: otherUser, source: 'Listing'),
        ),
      );
    } catch (e) {
