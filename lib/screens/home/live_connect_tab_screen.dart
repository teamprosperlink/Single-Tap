...
class LiveConnectTabScreenState extends ConsumerState<LiveConnectTabScreen>
  ...
  final List<String> _tabCategories = ['SingleTap.png ', 'Smart '];
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging) {
        ...
        setState(() {
          if (selectedCategory == 'SingleTap.png ') {
  ...
  Future<void> _loadUserProfile() async {
    ...
    try {
      ...
      // Load user profile location from users (reject stale Mountain View)
      final userDoc = await _firestore.collection('users').doc(userId).get();
      if (userDoc.exists && mounted) {
        final profileData = userDoc.data();
        final baseCity = (profileData?['city'] as String? ?? '').toLowerCase();
        if (baseCity.contains('mountain view')) {
          profileData?.remove('latitude');
          profileData?.remove('longitude');
          profileData?.remove('city');
        }
        setState(() {
