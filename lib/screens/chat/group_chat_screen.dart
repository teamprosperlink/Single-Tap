...
class _GroupChatScreenState extends ConsumerState<GroupChatScreen>
  ...
  final List<Map<String, dynamic>> _olderMessages = []; // Stores paginated older messages
  ...
  // Cached active call stream to prevent permission-denied retry spam
  StreamController<QuerySnapshot>? _activeCallController;
  StreamSubscription<QuerySnapshot>? _activeCallSub;
  late final Stream<QuerySnapshot> _activeCallStream;
  bool _activeCallStreamError = false;
  ...
  void initState() {
    ...
    // Cache active call stream — cancel subscription on first error to stop retry spam
    _activeCallController = StreamController<QuerySnapshot>.broadcast();
    _activeCallSub = _firestore
        .collection('group_calls')
        .where('groupId', isEqualTo: widget.groupId)
        .snapshots()
        .listen(
          (data) {
            if (!_activeCallController!.isClosed) {
              _activeCallController!.add(data);
            }
          },
          onError: (error) {
            debugPrint('Active call stream error (subscription cancelled): $error');
            _activeCallStreamError = true;
    ...
    _activeCallStream = _activeCallController!.stream;
  ...
  Future<void> _loadMoreMessages() async {
    ...
    try {
      ...
      if (snapshot.docs.isEmpty) {
        ...
        // Store older messages for display
  ...
  Future<void> _sendMessage() async {
    ...
    // IMMEDIATELY lock to prevent any double-send from onSubmitted + onTap race
    _isSending = true;
  ...
  void _handleMentionDetection(String text) {
    ...
    for (final entry in _memberNames.entries) {
      ...
      if (query.isEmpty || entry.value.toLowerCase().contains(query)) {
        suggestions.add({
          'id': entry.key,
          'name': entry.value,
  ...
  List<Map<String, String>> _extractMentions(String text) {
    ...
    for (final match in matches) {
      ...
      final mentionedName = matchedText.substring(1); // remove @ prefix

      // Check if it's the AI mention
      if (RegExp(r'Single\s*Tap\s+AI', caseSensitive: false).hasMatch(mentionedName)) {
        mentions.add({
          'userId': ApiConfig.singletapAiSenderId,
          'name': 'Single Tap AI',
        });
        continue;
      }

      // Find user by name from members
      final entry = _memberNames.entries.firstWhere(
        (e) => e.value.toLowerCase() == mentionedName.toLowerCase(),
  ...
  Future<void> _startGroupAudioCall() async {
    ...
    try {
      // Check for active calls and cleanup stale ones (more aggressive: 2 minutes)
      final twoMinutesAgo = DateTime.now().subtract(const Duration(minutes: 2));

      debugPrint('🔍 Checking for active calls for user: $currentUserId');
      ...
      final List<String> callsToCleanup = [];

      for (var doc in activeCallsQuery.docs) {
        final callId = doc.id;
        ...
        if (timestamp == null || timestamp.toDate().isBefore(twoMinutesAgo)) {
          debugPrint('    → STALE (cleaning up)');
          callsToCleanup.add(callId);
          continue;
        }

        // Check participant's actual active status
        try {
          ...
          final isActive = participantDoc.data()?['isActive'] as bool? ?? false;
          debugPrint('    → User isActive in call: $isActive');

          if (isActive) {
            userIsReallyInActiveCall = true;
            debugPrint('    → ⚠️ User IS actively in this call');
            break; // Found active call, stop checking
          } else {
            debugPrint('    → User left this call, marking for cleanup');
            callsToCleanup.add(callId);
          }
        } catch (e) {
          debugPrint(
      ...
      // Cleanup stale/left calls
      if (callsToCleanup.isNotEmpty) {
        debugPrint('🧹 Cleaning up ${callsToCleanup.length} calls');
        for (var callId in callsToCleanup) {
          try {
            await _firestore.collection('group_calls').doc(callId).set({
              'status': 'ended',
      ...
      // Block only if user is TRULY active in a call
      if (userIsReallyInActiveCall) {
        debugPrint('⚠️ User actively in call, blocking new call');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
      ...
      debugPrint('    Creating group call with:');
      debugPrint('    - callerId: $currentUserId');
      debugPrint('    - participants: $uniqueParticipantIds');
      ...
      // Add optimistic call system message to UI immediately (Single Tap style)
      ...
      // Send system message to chat - Single Tap style with caller info
      ...
      // Send notifications to all participants (for background/closed app)
      debugPrint('  Sending notifications to participants...');
      final notificationService = NotificationService();

      for (final member in uniqueMembers) {
        final memberId = member['id'] as String;
        // Don't send notification to caller
        if (memberId != currentUserId) {
          try {
            await notificationService.sendNotificationToUser(
              userId: memberId,
              title: '${_memberNames[currentUserId] ?? 'Someone'} is calling',
              body: widget.groupName,
              type: 'group_call',
              data: {
                'callId': callDoc.id,
                'callerId': currentUserId,
                'callerName': _memberNames[currentUserId] ?? 'Someone',
                'callerPhoto': _memberPhotos[currentUserId],
                'groupId': widget.groupId,
                'groupName': widget.groupName,
              },
            );
            debugPrint('  Notification sent to: $memberId');
          } catch (e) {
      ...
      // detect this new call and show CallKit UI to all participants (when app is open)
      debugPrint(
        '    Group call created. Notifications sent to ${uniqueMembers.length - 1} participants.',
      );
  ...
  Future<void> _loadDailyMediaCounts() async {
    ...
    if (currentUserId == null) {
      debugPrint('    Cannot load: currentUserId is null, will retry later');
      _isCounterLoaded =
    ...
    if (_isCounterLoaded) {
      debugPrint('  Counter already loaded, skipping duplicate load');
      return;
    }

    // Prevent concurrent loads - already loading
    if (_isCounterLoading) {
      debugPrint('  Counter load already in progress, skipping');
      return;
    }
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      // Send each image (don't decrement after - counter should stay)
  ...
  // Handle tap on call card to join ongoing call
  Future<void> _handleCallCardTap(String? callId, int? callDuration) async {
    if (callId == null) return;

    // If call has duration, it's ended - don't allow join
    ...
    try {
      // Check if call is still active in Firestore
      ...
      if (callStatus != 'active') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('This call is no longer active'),
              backgroundColor: Colors.orange,
              duration: Duration(seconds: 2),
            ),
          );
      ...
      final participantIds = List<String>.from(callData?['participants'] ?? []);
      final participants = participantIds.map((id) {
        return {
          'userId': id,
          'name': _memberNames[id] ?? 'Unknown',
          'photoUrl': _memberPhotos[id],
        };
      }).toList();

      // Join the call - navigate to group audio call screen
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
  ...
  /// Show Single Tap-style delete dialog with glass effect
  ...
  Widget _buildActiveCallBanner(bool isDarkMode) {
    // Skip rendering if the stream already failed with permission error
    if (_activeCallStreamError) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          debugPrint('Error loading active call: ${snapshot.error}');
          _activeCallStreamError = true;
          return const SizedBox.shrink();
        }
        ...
        // Find active call or return empty banner
        DocumentSnapshot? activeCallDoc;
        try {
          activeCallDoc = sortedDocs.firstWhere(
            (doc) => (doc.data() as Map<String, dynamic>)['status'] == 'active',
          );
        } catch (e) {
          return const SizedBox.shrink();
        }
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, participantSnapshot) {
            final activeParticipants = participantSnapshot.data?.docs ?? [];
            final count = activeParticipants.length;

            final activeUserIds = activeParticipants
                .map((doc) => doc['userId'] as String)
                .take(3)
                .toList();

            return GestureDetector(
              onTap: () => _joinActiveCall(callId, callData),
              child: Container(
                width: double.infinity,
                      ...
                      child: Stack(
                        children: [
                          ...
                          if (activeUserIds.isNotEmpty)
                            ...activeUserIds.asMap().entries.map((entry) {
                              final index = entry.key;
                              final userId = entry.value;
                              final photoUrl = _memberPhotos[userId];
  ...
  void _joinActiveCall(String callId, Map<String, dynamic> callData) async {
    final currentUserId = _currentUserId;
    if (currentUserId == null) return;

    final participantIds = List<String>.from(callData['participants'] ?? []);
    final participantsList = participantIds.map((id) {
      return {
        'userId': id,
        'name': _memberNames[id] ?? 'Unknown',
        'photoUrl': _memberPhotos[id],
    ...
    // If user is not in participants array, add them to Firestore
    if (!participantIds.contains(currentUserId)) {
      ...
      // Update Firestore call document to include this user
      try {
        await _firestore.collection('group_calls').doc(callId).update({
          'participants': FieldValue.arrayUnion([currentUserId]),
        });
        debugPrint('Added $currentUserId to call participants array');
      } catch (e) {
        debugPrint('Error adding user to participants array: $e');
      }
    }
  ...
  String _getTypingText() {
    ...
    final currentUserId = _currentUserId;
    final isAiTyping = _typingUsers.contains(ApiConfig.singletapAiSenderId);
    final typingNames = _typingUsers
        .where((id) => id != currentUserId && id != ApiConfig.singletapAiSenderId)
    ...
    if (typingNames.length == 1) {
      parts.add('${typingNames[0]} is typing...');
    } else if (typingNames.length == 2) {
      parts.add('${typingNames.join(' and ')} are typing...');
    } else if (typingNames.isNotEmpty) {
      parts.add('${typingNames.length} people are typing...');
    }

    if (isAiTyping) {
      parts.add('Single Tap AI is thinking...');
    }
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData && snapshot.data!.exists) {
          ...
          _memberNames = (data['participantNames'] as Map<String, dynamic>? ?? {}).map(
            (k, v) => MapEntry(k, v?.toString() ?? 'Unknown'),
          );
          _memberPhotos = (data['participantPhotos'] as Map<String, dynamic>? ?? {}).map(
            (k, v) => MapEntry(k, v?.toString()),
          );

          final isTypingMap = (data['isTyping'] as Map<String, dynamic>? ?? {}).map(
              ...
              SafeArea(
                ...
                child: Column(
                  children: [
                      ...
                      child: Container(
                        ...
                        child: StreamBuilder<QuerySnapshot>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            // Remove optimistic messages that now exist in Firestore (to prevent duplicates)
                            final firestoreMessageIds = messages
                                .map((doc) => doc.id)
                                .toSet();
                            final dedupedOptimisticMessages =
                                _optimisticMessages
                                    .where(
                                      (msg) => !firestoreMessageIds.contains(
                                        msg['id'],
                                      ),
                                    )
                                    .toList();

                            // Merge stream messages + older paginated messages
                            final streamMsgIds = messages.map((doc) => doc.id).toSet();
                            final dedupedOlderMessages = _olderMessages
                                .where((m) => !streamMsgIds.contains(m['id']))
                                .toList();

                            var allMessages = [
                              ...dedupedOptimisticMessages, // Newest (optimistic, not yet in Firestore)
                              ...messages.map(
                                (doc) => {
                                  'id': doc.id,
                              ...
                              ...dedupedOlderMessages, // Then older paginated messages
                            ];
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                if (index < allMessages.length) {
                                  ...
                                  // With reverse: true, index+1 is the older message above
                                  Widget? dateSeparator;
                                  final currentDate = timestamp?.toDate();
                                  if (index == allMessages.length - 1) {
                                    // Oldest visible message - always show its date
                                    dateSeparator = _buildDateSeparator(currentDate, isDarkMode);
                                  } else {
                                    final nextTimestamp = allMessages[index + 1]['timestamp'] as Timestamp?;
                                  ...
                                  // Build the appropriate message widget
                                  Widget messageWidget;

                                  // Handle AI messages
                                  final isAiMessage = senderId == ApiConfig.singletapAiSenderId ||
                                      messageData['isAiMessage'] == true;

                                  if (isAiMessage) {
                                    messageWidget = _buildAiMessageBubble(
                                      text: text,
                                      timestamp: timestamp,
                                      isDarkMode: isDarkMode,
                                    );
                                  } else if (isSystemMessage) {
                                    messageWidget = _buildSystemMessage(
                                      text,
                                      isDarkMode,
                                      messageData,
                                    );
                    ...
                    _buildAiThinkingIndicator(isDarkMode),
                    // Reply preview
      ...
      actions: [
        if (_isMultiSelectMode) ...[
          ...
          IconButton(
            icon: Icon(
              Icons.call_rounded,
              color: _isStartingCall
                  ? Colors.white70.withValues(alpha: 0.5)
                  : Colors.white70, // Always white for solid dark appbar
              size: 24,
            ),
            onPressed: _isStartingCall ? null : _startGroupAudioCall,
            tooltip: 'Voice Call',
          ),
        ],
        if (!_isMultiSelectMode)
  ...
  Future<void> _uploadAndSendVoice(
    ...
    try {
      // Upload to Firebase Storage (using chat_media path which has storage rules)
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final fileName = 'voice_${widget.groupId}_$timestamp.aac';
  ...
  Widget _buildAiThinkingIndicator(bool isDarkMode) {
    final isAiTyping = _typingUsers.contains(ApiConfig.singletapAiSenderId);
    if (!isAiTyping) return const SizedBox.shrink();
          ...
          Flexible(
            child: Column(
              ...
              children: [
                ...
                // AI message bubble with soft indigo tint
                IntrinsicWidth(
  ...
  Widget _buildSystemMessage(
    ...
    if (actionType == 'call') {
      ...
      if (isMissed) {
        ...
        final prefix = isCallerCurrentUser ? 'Outgoing call' : 'Incoming call';
        callText = '$prefix • $durationText';
      }

      // Single Tap-style positioning: right for caller, left for others
      return GestureDetector(
            ...
            children: [
              ...
              Column(
                crossAxisAlignment: isCallerCurrentUser
                    ? CrossAxisAlignment
                          .end // My call content → RIGHT
                    : CrossAxisAlignment.start, // Others' content → LEFT
                mainAxisSize: MainAxisSize.min,
                children: [
                  GestureDetector(
                    onTap: _isMultiSelectMode && messageId != null
                  ...
                  ), // Close IntrinsicWidth + ConstrainedBox + Container + GestureDetector
                  ),
                  // Timestamp below the call card (same style as chat messages)
                  if (timestamp != null)
                    Padding(
                  ...
                  // Show "Join" button ONLY if someone has already joined the call
                  if (callId != null &&
  ...
  // Build date separator widget - same style as 1-on-1 chat
        ...
        child: Container(
          ...
          child: Row(
            mainAxisAlignment: isMe
                ? MainAxisAlignment
                      .end // My messages on RIGHT
                : MainAxisAlignment.start, // Others' messages on LEFT
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // Selection checkbox
              if (_isMultiSelectMode) ...[
              ...
              Flexible(
                child: Column(
                  ...
                  crossAxisAlignment: isMe
                      ? CrossAxisAlignment
                            .end // My messages aligned RIGHT
                      : CrossAxisAlignment
                            .start, // Others' messages aligned LEFT
                  children: [
                    IntrinsicWidth(
                      child: ConstrainedBox(
                        constraints: BoxConstraints(
                        ...
                        child: ClipRRect(
                          ...
                          child: Container(
                              ...
                              child: Column(
                                crossAxisAlignment: isMe
                                    ? CrossAxisAlignment
                                          .end // My messages content aligned RIGHT
                                    : CrossAxisAlignment
                                          .start, // Others' content aligned LEFT
                                children: [
                                  // Sender name at top inside card (for received messages - text only)
                                  if (!isMe &&
                                      imageUrl == null &&
                                      videoUrl == null &&
                                      voiceUrl == null) ...[
  ...
  Widget _buildAudioMessagePlayer({
    ...
    // Audio player Single Tap style - glass background
    return Container(
      width: MediaQuery.of(context).size.width * 0.7,
      decoration: BoxDecoration(
        gradient: LinearGradient(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          // Username for receiver messages
          if (!isMe) ...[
            Container(
              width: double.infinity,
  ...
  // Build message status icon (Single Tap-like ticks)
  Widget _buildMessageStatusIcon(
    ...
    // Single Tap group chat tick logic:
                    ...
                    child: Chip(
                      ...
                      avatar: CircleAvatar(
                        ...
                        child: !PhotoUrlHelper.isValidUrl(user['photoUrl'])
                            ? Text(
                                ((user['name']?.toString() ?? 'U').isNotEmpty ? (user['name']?.toString() ?? 'U')[0] : 'U').toUpperCase(),
                      ...
                      return ListTile(
                        leading: CircleAvatar(
                          ...
                          child: !PhotoUrlHelper.isValidUrl(contact['photoUrl'])
                              ? Text(
                                  ((contact['name']?.toString() ?? 'U').isNotEmpty ? (contact['name']?.toString() ?? 'U')[0] : 'U').toUpperCase(),
