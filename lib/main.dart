...
void main() async {
  ...
  // Catch platform-level errors (JNI image decode failures, etc.)
  PlatformDispatcher.instance.onError = (error, stack) {
    final errorStr = error.toString();
    // Suppress non-fatal image decode errors at the platform level
    if (errorStr.contains('ImageDecoder') ||
        errorStr.contains('Failed to decode image') ||
            ...
            (snapshot) async {
              try {
                ...
                if (!serverTokenValid && localTokenValid) {
                  debugPrint('[DeviceSession]  TOKEN CLEARED ON SERVER');
                  if (mounted && !_isPerformingLogout) {
                    _isPerformingLogout = true;
                    await _performRemoteLogout('Another device logged in');
                  }
